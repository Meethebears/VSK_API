...
namespace API.Controllers
{
    public class PackingController : ApiController
    {
        ...
        [Route("v1/Trp_Packing_Job_Mm_Get")]
        [HttpGet]
        public ResponseModel Trp_Packing_Job_Mm_Get([FromUri] TempModel TempModel)
        {
            try
            {
                PackingRepository PackingRepository = new PackingRepository();

                List<TempModel> Trp_Packing_Job_Mm_Get = PackingRepository.Trp_Packing_Job_Mm_Get(TempModel);

                ResponseModel _ResponseModel = new ResponseModel();

                _ResponseModel.result_datetime = DateTime.Now.ToString("yyyy-MM-dd hh:mm");
                _ResponseModel.data = Trp_Packing_Job_Mm_Get;
                _ResponseModel.length = Trp_Packing_Job_Mm_Get.Count();
                _ResponseModel.status = "Success";
        ...
        [Route("v1/Trp_Packing_Check_Box_Update_Mm")]
        [HttpPut]
        public ResponseModel Trp_Packing_Check_Box_Update_Mm(TempModel TempModel)
        {
            try
            {
                PackingRepository PackingRepository = new PackingRepository();

                List<TempModel> Trp_Packing_Check_Box_Update_Mm = PackingRepository.Trp_Packing_Check_Box_Update_Mm(TempModel);

                ResponseModel _ResponseModel = new ResponseModel();

                _ResponseModel.result_datetime = DateTime.Now.ToString("yyyy-MM-dd hh:mm");
                _ResponseModel.data = Trp_Packing_Check_Box_Update_Mm;
                _ResponseModel.length = Trp_Packing_Check_Box_Update_Mm.Count();
                _ResponseModel.status = "Success";
