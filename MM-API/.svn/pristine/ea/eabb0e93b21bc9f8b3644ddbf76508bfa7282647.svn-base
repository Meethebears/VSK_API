...
namespace REPO.Controllers
{
    public class DashbordRepository : Controller
    {
        ...
        public IList<DashboardPKDailyDetailModel> DashboardPKDailyDetail_GET(string INVDATE, string ORDERTYPE, string STARTTIME, string ENDTIME)
        {
            try
            {
                ...
                IList<DashboardPKDailyDetailModel> DashboardPKDailyDetail_list = VSK_DASHBOARD.Query<DashboardPKDailyDetailModel>("SP_PK_DAILY_DETAIL_GET", objParam, commandTimeout: 210, commandType: CommandType.StoredProcedure).ToList();

                VSK_DASHBOARD.Close();
                return DashboardPKDailyDetail_list.ToList();
