...
namespace API.Controllers
{
    public class DashboardController : ApiController
    {
        ...
        [Route("v4/DashboardPKDailyDetail_Get")]
        [HttpGet]
        public ResponseModel DashboardPKDailyDetail_GET([FromUri] string INVDATE, [FromUri] string ORDERTYPE, [FromUri] string STARTTIME, [FromUri] string ENDTIME)
        {
            try
            {
                DashbordRepository DashbordRepository = new DashbordRepository();

                List<DashboardPKDailyDetailModel> DashboardPKDailyDetail_GET = (List<DashboardPKDailyDetailModel>)DashbordRepository.DashboardPKDailyDetail_GET(INVDATE, ORDERTYPE, STARTTIME, ENDTIME);

                ResponseModel _ResponseModel = new ResponseModel();
                _ResponseModel.result_datetime = DateTime.Now.ToString("yyyy-MM-dd hh:mm");
                _ResponseModel.data = DashboardPKDailyDetail_GET;
                _ResponseModel.length = DashboardPKDailyDetail_GET.Count();
                _ResponseModel.status = "Success";
